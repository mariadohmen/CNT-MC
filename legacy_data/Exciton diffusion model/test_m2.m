clear all
r_ex_li=linspace(0,5,21);  %radius of exciton
n_def_li=linspace(0,100,101); %number of defects on nanotube
p_ph2_li=linspace(0.02,0.8,40);
int=0;         
k_r=10^(10);
k_nr=[10^(11)];
long=260;
...
n_ex=10;                 %number of excitons !will some day be calculated!
...
fin=spw+(direc*pass)-r_ex;
 ...
 for zr=1:numel(tramid(:,1))
    if tramid(zr,2)<=2*r_ex
        tramid(zr,2)=1;
        tramid(zr,3)=1;
    elseif tramid(zr,2)>2*r_ex
        tramid(zr,2)=0;
        tramid(zr,3)=0;
    end
 end
...
%         while br <= 0;
        if tramid(mc,2)==1
            ...
            br = 1;
            break
        elseif tramid (mc,2)==0
        nu_def=nu_def+1;
        ph1=0;
        if nu_def==2 && tramid(mc,1)<=fin
            supi=rand;
            br = 1;
            if supi <=p_ph2
                int2=int2+1;
...
%             br = 1;
                        break
        elseif tramid(nc,3)==0
        nu_def=nu_def+1;
        ph1=0;
        if nu_def==2 && tramid(nc,1)>=fin
            supi=rand;
            if supi <=p_ph2
                int2=int2+1;
                br = 1;
...
str1 = ['upm_2n' num2str(rrex) '_rrex.txt'];
dlmwrite(str1, upm(rrex,:,:))
end
